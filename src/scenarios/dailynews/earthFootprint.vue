...
const centerPosition = (svg, projection, path, state, animate = true) => {
  ...
  const locate2position = () => {
    ...
    svg
      .transition()
      .duration(duration) // 设置动画持续时间为 2.5 秒
      .tween("rotate", () => {
...
const createMarker = (svg, projection, path, point, locationName = "") => {
  ...
  svg
    ...
    .attr("height", iconSize)
    .html(
      `<div id='city' style='display:inline-block; white-space: nowrap; font-size:12px; color:rgba(0, 0, 0, 1);'>${locationName}</div>`
    );
  svg.selectAll("path").attr("d", path);
  const container = document.getElementById("marker");
  const icon = h(EnvironmentTwoTone, {
    spin: false,
    rotate: 0,
    style: { fontSize: "30px" },
  });
